$(() => {
  ...
  $("form").on("submit", event => {
    ...
    for (let i = 0; i < centreInfo.result.records.length; i++) {
      if (
        centreInfo.result.records[i].centre_name
          .toLowerCase()
          .includes(inputName)
      ) {
    ...
    for (let i = 0; i < selectedSchools.length; i++) {
      ...
      let $centreName = $("<div>")
        .text(selectedSchools[i].centre_name)
        .addClass("centreName");
      ...
      let $centreContact = $("<li>").text(
        "Telephone: " + selectedSchools[i].centre_contact_no
      );
      $centreInfoList.append($centreContact);

      let $centreEmail = $("<li>").text(
        "Email: " + selectedSchools[i].centre_email_address
      );
    ...
    $(".centreName").on("click", event => {
      targetCentreName = $(event.currentTarget)
        .text()
        .toLowerCase();
      ...
      for (let i = 0; i < selectedSchools.length; i++) {
        if (targetCentreName == selectedSchools[i].centre_name.toLowerCase()) {
          ...
          targetCentre.organisation_description =
            selectedSchools[i].organisation_description;
          ...
          targetCentre.centre_email_address =
            selectedSchools[i].centre_email_address;
          targetCentre.weekday_full_day = selectedSchools[i].weekday_full_day;
          targetCentre.extended_operating_hours =
            selectedSchools[i].extended_operating_hours;
          targetCentre.provision_of_transport =
            selectedSchools[i].provision_of_transport;
          targetCentre.gst_regisration = selectedSchools[i].gst_regisration;
          targetCentre.scheme_type = selectedSchools[i].scheme_type;
          targetCentre.second_languages_offered =
            selectedSchools[i].second_languages_offered;
      ...
      for (let i = 0; i < centreFee.result.records.length; i++) {
        if (
          targetCentre.centre_code == centreFee.result.records[i].centre_code &&
          centreFee.result.records[i].type_of_citizenship == "SC" &&
          centreFee.result.records[i].type_of_service == "Full Day"
        ) {
          if (
            centreFee.result.records[i].levels_offered ==
            "Infant (2 to 18 mths)"
          ) {
            ...
            centreFee.result.records[i].levels_offered == "Nursery (4 yrs old)"
          ) {
      ...
      for (let i = 0; i < centreLicense.result.records.length; i++) {
        if (
          targetCentre.centre_code ==
          centreLicense.result.records[i].centre_code
        ) {
          targetCentre.license_tenure =
            centreLicense.result.records[i].license_tenure;
          targetCentre.license_issue_date =
            centreLicense.result.records[i].license_issue_date;
        }
      }
      ...
      let $targetCentreName = $("<div>")
        .text(targetCentre.centre_name)
        .addClass("targetCentreName");
      ...
      let $targetCentreAddress = $("<div>")
        .text(targetCentre.centre_address)
        .addClass("targetCentreInfoItem");
      $targetCentreInfo.append($targetCentreAddress);

      let $targetCentreContact = $("<div>")
        .text("Telephone: " + targetCentre.centre_contact_no)
        .addClass("targetCentreInfoItem");
      $targetCentreInfo.append($targetCentreContact);

      let $targetCentreEmail = $("<div>")
        .text("Email: " + targetCentre.centre_email_address)
        .addClass("targetCentreInfoItem");
      ...
      $column1Title = $("<div>")
        .text("School Operation")
        .addClass("columnTitle");
      $targetCentreWeekday = $("<div>").text(
        "Weekday Operating Hours: " + targetCentre.weekday_full_day
      );
      $targetCentreExtended = $("<div>").text(
        "Extended Hours: " + targetCentre.extended_operating_hours
      );
      $targetCentreTransport = $("<div>").text(
        "Transport Service: " + targetCentre.provision_of_transport
      );
      $targetCentreGST = $("<div>").text(
        "GST Charge: " + targetCentre.gst_regisration
      );
      $targetCentreScheme = $("<div>").text(
        "Extra Financial Support: " + targetCentre.scheme_type
      );
      $targetCentreLanguage = $("<div>").text(
        "Second Language Offered: " + targetCentre.second_languages_offered
      );
      ...
      $column2Title = $("<div>")
        .text("School Fee")
        .addClass("columnTitle");
      $targetCentreInfantFee = $("<div>").text(
        "Infant Care Fee: " + targetCentre.infant_fee
      );
      $targetCentrePlaygroupFee = $("<div>").text(
        "Playgroup Fee: " + targetCentre.playgroup_fee
      );
      $targetCentrePrenurseryFee = $("<div>").text(
        "Pre-Nursery Fee: " + targetCentre.pre_nursery_fee
      );
      $targetCentreNurseryFee = $("<div>").text(
        "Nursery Fee: " + targetCentre.nursery_fee
      );
      $targetCentreKindergarten1Fee = $("<div>").text(
        "Kindergarten 1 Fee: " + targetCentre.kindergarten1_fee
      );
      $targetCentreKindergarten2Fee = $("<div>").text(
        "Kindergarten 2 Fee: " + targetCentre.kindergarten2_fee
      );
      ...
      $column3Title = $("<div>")
        .text("School License")
        .addClass("columnTitle");
      $targetCentreSpark = $("<div>").text(
        "SPARK Certification: " + targetCentre.spark_certified
      );
      $targetCentreLicenseTenure = $("<div>").text(
        "License Tenure: " + targetCentre.license_tenure
      );
      $targetCentreLicenseDate = $("<div>").text(
        "License Issue Date: " + targetCentre.license_issue_date
      );
